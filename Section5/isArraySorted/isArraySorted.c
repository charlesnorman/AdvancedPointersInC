...
int isArraySortedRecursive(int *arr, int *reallySorted, int size)
{
     int result;
     if (size == 1)
     {
          *reallySorted = 1; //Assume really sorted
          return 1;
     }

     result = isArraySortedRecursive(arr, reallySorted,size -1);
     /*If the array is Sorted/Really Sorted and the
     last elements are equal*/
     if(result != 0 && arr[size-1] == arr[size-2])
     {
          *reallySorted = 0;
     }
     /** If the array is Sorted/Sorted and number on left
      * is less than number on right = not sorted
     */
     if(result != 0 && arr[size-1] < arr[size-2])
     {
          *reallySorted = 0;
...
int main()
{
     ...
     sorted = isArraySortedRecursive(arr1, &reallySorted, sizeof(arr1) / sizeof(int));
     printf("Is arr1 \"Sorted\"? %s Is arr1 Is arr1 \"Really Sorted\"? %s \n", (sorted == 1) ? "Yes" : "No", (reallySorted == 1) ? "Yes" : "No");

     sorted = isArraySortedRecursive(arr2, &reallySorted, sizeof(arr2) / sizeof(int));
     printf("Is arr2 \"Sorted\"? %s Is arr2 Is arr1 \"Really Sorted\"? %s \n", (sorted == 1) ? "Yes" : "No", (reallySorted == 1) ? "Yes" : "No");

     sorted = isArraySortedRecursive(arr3, &reallySorted, sizeof(arr3) / sizeof(int));
